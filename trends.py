...
def find_center(shapes):
    ...
    """
    "*** YOUR CODE HERE ***"
    ax = sum(x[0] * x[2] for x in tuple(map(find_centroid, shapes))) / sum(x[2] for x in tuple(map(find_centroid, shapes)))
    by = sum(x[1] * x[2] for x in tuple(map(find_centroid, shapes))) / sum(x[2] for x in tuple(map(find_centroid, shapes)))
    return make_position(ax, by)
...
def find_closest_state(tweet, state_centers):
    ...
    """
    "*** YOUR CODE HERE ***"
    loca = tweet_location(tweet)
    states_to_dists = {}
    for state in state_centers:
        states_to_dists[state] = geo_distance(loca, state_centers[state])
    dists_to_states = {dist: state for state, dist in states_to_dists.items()}
    return dists_to_states[min(dists_to_states.keys())]
...
def group_tweets_by_state(tweets):
    ...
    us_centers = {n: find_center(s) for n, s in us_states.items()}
    for tweet in tweets:
        match = find_closest_state(tweet, us_centers)
        if match not in tweets_by_state:
            tweets_by_state[match] = list()
        tweets_by_state[match].append(tweet)
...
def calculate_average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        scores = [analyze_tweet_sentiment(tweet) for tweet in tweets_by_state[state] if analyze_tweet_sentiment(tweet) != None]
        if len(scores) > 0:
            averaged_state_sentiments[state] = sum(scores) / len(scores)
